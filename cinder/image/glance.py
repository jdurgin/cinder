...
def _create_glance_client(context, host, port, version):
...
def api_server_from_catalog(service_catalog,
                            service_type=FLAGS.glance_service_type,
                            endpoint_region=FLAGS.glance_endpoint_region,
                            endpoint_type=FLAGS.glance_endpoint_type):
    """
    Select a Glance API endpoint from the service catalog

    :returns: (host, port) of the Glance API server to use.
    """
    endpoint = None
    ...
    if endpoint and endpoint.get(endpoint_type):
        o = urlparse.urlparse(endpoint.get(endpoint_type))
        port = o.port or 9292
        host = o.netloc.split(':', 1)[0]
        return host, port
    else:
...
class GlanceClientWrapper(object):
    ...
    def __init__(self, context=None, host=None, port=None, version=None):
        if host is not None and port is not None:
            self.client = self._create_static_client(context, host, port, version)
        else:
            self.client = None
    ...
    def _create_static_client(self, context, host, port, version):
        ...
        return _create_glance_client(context, self.host, self.port, self.version)

    def _create_onetime_client(self, context, version):
        """Create a client that will be used for one call."""
        if FLAGS.glance_service_catalog:
            catalog = context.service_catalog
            self.host, self.port = api_server_from_catalog(catalog)
        else:
...
class GlanceImageService(object):
    ...
    def get_location(self, context, image_id):
        ...
        if hasattr(image_meta, 'direct_url'):
            return image_meta.direct_url
        return None
